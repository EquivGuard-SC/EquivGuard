...
def static_parser(file_path):
    ...
    cmd2 = f"slither {arg1} --detect constChainID,constant-address,constBlocknumber,operateBlocknumber,gasleft,reentrancy-unlimited-gas --solc-disable-warnings --json {file_name}"
    ...
    with open('detectiion_output/stage1_detection.txt','a') as f:
        if static_detect:
            f.write(arg1+',stage1!,'+str(run_time))
            f.write('\n')
        else :
            f.write(arg1+',stage1_No!,'+str(run_time))
            f.write('\n')
