...
`send` and `transfer` do not protect from reentrancies in case of gas price changes."""
    ...
    def find_reentrancies(self) -> DefaultDict[FindingKey, Set[FindingValue]]:
        result: DefaultDict[FindingKey, Set[FindingValue]] = defaultdict(set)
        keywords = ['nonReentrant', 'guard']
        pattern = re.compile('|'.join(keywords), re.IGNORECASE)
        for contract in self.contracts:
            for f in contract.functions_and_modifiers_declared:
                
                all_functions = (
                    [f for f in f.all_internal_calls() if isinstance(f, Function)]
                    + [f]
                    + [m for m in f.modifiers if isinstance(m, Function)] )
                ...
                if f.visibility in ["external", "public"]:
                    # print("qqq",contract,f,f.modifiers)    
                    matched = any(bool(pattern.search(str(mod.full_name))) for mod in f.modifiers if f.modifiers is not None)
                    match_mod = False
                    for mod in f.modifiers:
                        for nm in mod.nodes:
                            for mv in nm.variables_read:  # 假设每个modifier都有一个variables属性
                                # print("mv", mv)
                                if is_dependent(mv, SolidityVariableComposed("msg.sender"), node):
                                    match_mod = True
                    if not matched and not match_mod:  
                        for node in f.nodes:
                            has_control_flow_statements = node.contains_if() or node.contains_require_or_assert()
                            no_independent_of_sender = True  # 假设所有变量都不依赖于"msg.sender"，直到证明并非如此

                            if node.contains_if() or node.contains_require_or_assert():
                                for v in node.slithir_variables:
                                    if not isinstance(v, (Variable, SolidityVariable)):
                                        continue
                                    # 检查变量是否依赖于 SolidityVariableComposed("msg.sender")
                                    if is_dependent(v, SolidityVariableComposed("msg.sender"), node):
                                        # print("xxx",f,node)
                                        # for vv in node.variables_read:
                                            # print("vvv",vv.name)
                                        no_independent_of_sender = False
                                        break  # 找到一个不依赖于"msg.sender"的变量，无需进一步检查

                            # 根据条件打印结果
                            if not has_control_flow_statements or (has_control_flow_statements and not no_independent_of_sender):
                                # print("\tvar_read:")
                                if self.KEY not in node.context:
                                    continue
                                if node.context[self.KEY].calls:
