...
def _valid_candidate(v: StateVariable) -> bool:
    ...
    return _is_valid_type(v) or v.is_constant
...
class UnchangedStateVariables:
    ...
    def detect_const_add(self) -> None:
        """Detect state variables that could be constant or immutable"""
        for c in self.compilation_unit.contracts_derived:
            if c.is_signature_only():
                continue
            ...
            for v in valid_candidates:
                if v not in variables_written:
                    if _constant_initial_expression(v) and v not in constructor_variables_written and v.type == ElementaryType("address"):
                    # if v not in constructor_variables_written and v.type == ElementaryType("address"):
                        self.constant_candidates.append(v)
            ...
            #     for variable in function.local_variables:
                ...
                #                 # print("vvv",variable.type)
                #                 # print("vvv",node.variables_read_as_expression)
                #             # if (variable.type is not None):
                #             print("vvv----",variable)
                #             self.constant_candidates.append(variable)